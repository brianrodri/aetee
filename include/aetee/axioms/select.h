...
struct selectFunctor {
    ...
    constexpr decltype(auto) operator()(idx_t<I> i, A&&... args) const
    {
    ...
    constexpr decltype(auto) operator()(bool_t<C>, A&& a, B&& b) const
    {
...
private:
    ...
    static constexpr decltype(auto) impl(idx_t<0>, H&& head, T&&...)
    {
        return std::forward<H>(head);
    }

    template <size_t I, typename H, typename... T>
    static constexpr decltype(auto) impl(idx_t<I>, H&&, T&&... tail)
    {
...
private:
    ...
    constexpr decltype(auto) impl(idx_c_sequence_t<I...>, X&&... x)
    {
    ...
    template <typename... X, size_t... I>
    constexpr decltype(auto) impl(idx_c_sequence_t<I...>, X&&... x) const
    {
        return selectFunctor{}(
            p(std::forward<X>(x)...)
          , std::get<I>(transformers)...
